...
describe('Cron defaultValue test suite', function () {
  var defaultError = {
  ...
  var cases = [{
    ...
    title: 'leading zero is added for hours and minutes with clock format 24 hours',
    defaultValue: '1 3,18 6,23 * *',
    ...
    title: 'that undefined for default value is considered like an empty string',
    ...
    title: 'that an empty string is allowed for default value with allowEmpty always',
    defaultValue: '',
    allowEmpty: 'always',
    periodSelect: 'day',
    monthsSelect: undefined,
    ...
    title: 'that en empty string is allowed for default value with allowEmpty for-default-value',
    defaultValue: '',
    ...
    title: 'that an empty string is not allowed for default value with allowEmpty never',
    defaultValue: '',
    allowEmpty: 'never',
    periodSelect: 'day',
    monthsSelect: undefined,
    ...
    title: 'that monthly shortcut is not allowed when shortcuts only accept reboot',
    defaultValue: '@monthly',
    shortcuts: ['@reboot'],
    periodSelect: 'day',
    monthsSelect: undefined,
    ...
    title: 'that an out of range value too low for range first part is not allowed',
    defaultValue: '* * 0-3/4 * *',
    periodSelect: 'day',
    monthsSelect: undefined,
    monthDaysSelect: undefined,
    ...
    title: 'that a range first value greater than second value is not allowed',
    defaultValue: '* * * * 5-2',
    periodSelect: 'day',
    monthsSelect: undefined,
    monthDaysSelect: undefined,
  ...
  test.each(cases)('should check $title', function (_ref) {
    var defaultValue = _ref.defaultValue,
        expectedValue = _ref.expectedValue,
        expectedPeriod = _ref.expectedPeriod,
        allowEmpty = _ref.allowEmpty,
        shortcuts = _ref.shortcuts,
        humanizeLabels = _ref.humanizeLabels,
        humanizeValue = _ref.humanizeValue,
        leadingZero = _ref.leadingZero,
        clockFormat = _ref.clockFormat,
        allowedDropdowns = _ref.allowedDropdowns,
        defaultPeriod = _ref.defaultPeriod,
        periodSelect = _ref.periodSelect,
        monthsSelect = _ref.monthsSelect,
        monthDaysSelect = _ref.monthDaysSelect,
        weekDaysSelect = _ref.weekDaysSelect,
        hoursSelect = _ref.hoursSelect,
        minutesSelect = _ref.minutesSelect,
        error = _ref.error;
    ...
    if (defaultValue && !error) {
      var valueToCheck = expectedValue || defaultValue;
      var selectedPeriodToCheck = expectedPeriod || periodSelect;
    ...
    if (periodSelect) {
      expect(_react.screen.getByTestId('select-period')).toBeVisible();
      expect(_react.screen.getByTestId('select-period').textContent).toContain(periodSelect);
    } else {
      expect(_react.screen.queryByTestId(/select-period/i)).toBeNull();
    }

    if (monthsSelect) {
      expect(_react.screen.queryByTestId('custom-select-months')).toBeVisible();
      expect(_react.screen.getByTestId('custom-select-months').textContent).toContain(monthsSelect);
    } else {
      expect(_react.screen.queryByTestId(/custom-select-months/i)).toBeNull();
    }

    if (monthDaysSelect) {
      expect(_react.screen.queryByTestId('custom-select-month-days')).toBeVisible();
      expect(_react.screen.getByTestId('custom-select-month-days').textContent).toContain(monthDaysSelect);
    } else {
      expect(_react.screen.queryByTestId(/custom-select-month-days/i)).toBeNull();
    }

    if (weekDaysSelect) {
      expect(_react.screen.queryByTestId('custom-select-week-days')).toBeVisible();
      expect(_react.screen.getByTestId('custom-select-week-days').textContent).toContain(weekDaysSelect);
    } else {
      expect(_react.screen.queryByTestId(/custom-select-week-days/i)).toBeNull();
    }

    if (hoursSelect) {
      expect(_react.screen.queryByTestId('custom-select-hours')).toBeVisible();
      expect(_react.screen.getByTestId('custom-select-hours').textContent).toContain(hoursSelect);
    } else {
      expect(_react.screen.queryByTestId(/custom-select-hours/i)).toBeNull();
    }

    if (minutesSelect) {
      expect(_react.screen.queryByTestId('custom-select-minutes')).toBeVisible();
      expect(_react.screen.getByTestId('custom-select-minutes').textContent).toContain(minutesSelect);
    } else {
      expect(_react.screen.queryByTestId(/custom-select-minutes/i)).toBeNull();
    }
  });
});